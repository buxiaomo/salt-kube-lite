...
def parser(roles_yml):
    """
    parser the roles_yml from roles list to server list
    """
    roles_config = {}
    for role, servers in roles_yml.items():
        for server in servers:
            if server in roles_config.keys():
                roles_config[server].append(str(role))
            else:
                roles_config[server] = [str(role)]
    return roles_config
